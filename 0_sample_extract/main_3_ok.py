        ...
        try:
            ...
            with open(file_path, 'rb') as f:
                ...
                if header.startswith(b'PK\x03\x04'):
                    # Check DOCX/XLSX/PPTX signatures in the zip file
                    try:
                        import zipfile
                        with zipfile.ZipFile(file_path) as zf:
                            file_list = zf.namelist()
                            
                            # Check for specific files that indicate Office document types
                        ...
                        # If ZIP analysis fails, fall back to extension-based detection
                        pass
                    
                    # If no specific markers found, use extension or return zip
                    if ext in self.MIME_TYPES:
                        return self.MIME_TYPES[ext]
                    return 'application/zip'
                    
                elif header.startswith(b'\xD0\xCF\x11\xE0'):  # OLE2 file signature
                    if ext == '.doc':
                        return 'application/msword'
                    elif ext == '.xls':
                        return 'application/vnd.ms-excel'
                    ...
                    # Generic OLE document if extension doesn't match known types
                    return 'application/msword' if mime_type == 'application/octet-stream' else mime_type
            
            # Use the detected MIME type if it's not generic
                ...
                for proc in psutil.process_iter(['pid', 'name']):
                    try:
                        if proc.info['name'].upper() == process_name:
                            proc.terminate()
                            try:
                                proc.wait(timeout=0.5)  # Use wait instead of sleep
                            except:
                                pass
                            if proc.is_running():
                                proc.kill()
        ...
        for attempt in range(max_retries):
            try:
                ...
                if not self._init_word_app():
                    pythoncom.CoUninitialize()  # Clean up if initialization fails
                ...
                if attempt < max_retries - 1:
                    ...
                    self._cleanup_word_app()
                    # Make sure to uninitialize COM before retrying
                    try:
                        pythoncom.CoUninitialize()
                    except:
                        pass
