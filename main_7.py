...
class FileSignatureDetector:
    ...
    def inspect_ole_content(self, file_path: str) -> Optional[str]:
        ...
        try:
            ...
            # 检查PowerPoint标记
            if (
                "PowerPoint Document" in content_str
                or "Microsoft PowerPoint" in content_str
            ):
                return "application/vnd.ms-powerpoint"
    ...
    def get_all_files(self, directory: str) -> List[str]:
        ...
        for f in Path(directory).rglob("*"):
            if (
                ...
                and not self._is_internal_stream(f.name)
            ):
                ext = f.suffix.lower()
    ...
    def _detect_ole_file_type(self, file_path: str) -> str:
        ...
        try:
            ...
            with open(file_path, "rb") as f:
                ...
                if (
                    "Excel" in content_str
                    or "Worksheet" in content_str
                    or "Workbook" in content_str
                ):
                    return "application/vnd.ms-excel"
...
class ResultMonitor:
    ...
    def record_result(self, result: ProcessingResult):
        ...
        sensitive_words_list = "; ".join(
            [f"{w}({len(p)}处)" for w, p in result.sensitive_words]
        )
        ...
        status = (
            ...
            if result.error
                ...
                "空文件"
                if result.content.get("is_empty")
                else "已跳过"
                if result.content.get("skipped")
                else "成功"
            )
        )
...
class FileProcessor:
    ...
    def process_directory(self, directory: str) -> List[ProcessingResult]:
        ...
        print(f"\n开始处理目录: {directory}")
        print(
            f"共发现 {total_files} 个文件（已忽略 ~$ 开头、内部流文件及 .dwg/音频视频文件）"
        )
        print("=" * 80)
