    ...
    def _extract_doc_content(self, file_path: str) -> Dict[str, Any]:
        ...
        for attempt in range(max_retries):
            try:
                ...
                if attempt == max_retries - 1:
                    return self._create_error_result('doc', f"处理失败: {str(e)}")
        
        # 清理可能残留的进程
        self._force_close_office_processes('WINWORD.EXE')
        return self._create_error_result('doc', "所有处理尝试均失败")
            ...
            try:
                ...
                try:
                    ...
                    if attempt < max_retries - 1:
                        ...
                        time.sleep(2)
                        continue
                    return self._create_error_result('ppt', f"Win32COM 处理失败: {str(e)}")
                finally:
                    # 确保资源释放
                    try:
                        if presentation:
                ...
                if attempt == max_retries - 1:
                    return self._create_error_result('ppt', f"处理失败: {str(e)}")
        
        # 清理可能残留的进程
        self._force_close_office_processes('POWERPNT.EXE')
        return self._create_error_result('ppt', "所有处理尝试均失败")
...
class ResultExporter:
        ...
        export_data = [
            {
                'file_path': r.file_path,
                'mime_type': r.mime_type,
                'content_preview': r.text_content[:200],
                'sensitive_words': [{'word': w, 'positions': p} for w, p in r.sensitive_words],
                'error': r.error,
                'processing_time': r.processing_time
            } for r in results
    ...
    def export_to_excel(self, results: List[ProcessingResult], output_path: str):
        ...
        try:
            ...
            main_data = [
                {
                    ...
                    '处理时间(秒)': round(r.processing_time, 3),
                    '错误信息': r.error or '',
                    '内容预览': r.text_content[:500] + '...' if len(r.text_content) > 500 else r.text_content
                } for r in results
            ]
            ...
            logger.info(f"成功导出结果到Excel: {output_path}")
        except Exception as e:
            ...
            try:
                ...
                simple_output = output_path.replace('.xlsx', '_简化版.xlsx')
                pd.DataFrame(simple_data).to_excel(simple_output, index=False)
                logger.info(f"导出简化版Excel: {simple_output}")
            except Exception as e2:
