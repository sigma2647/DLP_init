...
class ContentExtractor:
    ...
    def _extract_xlsx_content(self, file_path: str) -> Dict[str, Any]:
        """提取XLSX文件内容，优化加密检测"""
        logger.info(f"处理XLSX文件: {file_path}")

        # 检查是否加密（XLSX通常是ZIP格式，加密时会有特定结构）
        try:
            import zipfile
            with zipfile.ZipFile(file_path) as zf:
                if zf.infolist() and zf.infolist()[0].flag_bits & 0x1:  # 检查加密标志
                    return {
                        "content": "",
                        "metadata": {"file_type": "xlsx", "status": "encrypted"},
                        "error": "涉密文件，跳过处理"
                    }
        except Exception as e:
            ...
            logger.debug(f"检查XLSX加密失败: {str(e)}")
        ...
        try:
            ...
            return {
                "content": "\n\n".join(content),
                "metadata": {"file_type": "xlsx", "extractor": "pandas"},
                "error": None
            }
        except Exception as e:
            ...
            return self._create_error_result("xlsx", f"Excel处理失败: {str(e)}")
    ...
    def _is_encrypted_xls(self, file_path: str) -> bool:
        """检查XLS文件是否加密"""

        try:
            import xlrd
            # 使用 on_demand=True，避免加载整个工作簿
            workbook = xlrd.open_workbook(file_path, on_demand=True)
            # 检查加密标记（xlrd 可能直接抛出异常）
            return False  # 如果成功打开且无加密标记，返回 False
        except xlrd.biffh.XLRDError as e:
            if "Workbook is encrypted" in str(e):
                return True
            return False
        except Exception:
            return False  # 其他异常不视为加密



    def _extract_xls_content(self, file_path: str) -> Dict[str, Any]:
        """提取XLS文件内容，简化逻辑并确保快速返回"""
        logger.info(f"处理XLS文件: {file_path}")

        # 提前检查加密
        if self._is_encrypted_xls(file_path):
            logger.info(f"提前检测到加密XLS文件: {file_path}")
            return {
                "content": "",
                "metadata": {"file_type": "xls", "status": "encrypted"},
                "error": "涉密文件，跳过处理"
            }
        ...
        try:
            import xlrd
            logger.debug(f"尝试使用xlrd打开工作簿: {file_path}")
            workbook = xlrd.open_workbook(file_path, on_demand=True)
            logger.debug(f"使用pandas读取内容: {file_path}")
            sheets = pd.read_excel(file_path, sheet_name=None, engine="xlrd")
            content = []
            for sheet_name, df in sheets.items():
                df = df.head(5000) if len(df) > 5000 else df
                sheet_content = f"Sheet: {sheet_name}\n{df.to_string(index=False)}"
                if len(df) > 5000:
                    sheet_content += "\n[注意: 表格过大，仅显示前5000行]"
                content.append(sheet_content)
            logger.info(f"成功提取XLS内容: {file_path}")
            workbook.release_resources()
            return {
                "content": "\n\n".join(content),
                "metadata": {"file_type": "xls", "extractor": "pandas-xlrd"},
                "error": None
            }
        except xlrd.biffh.XLRDError as e:
            if "Workbook is encrypted" in str(e):
                logger.info(f"确认检测到加密XLS文件: {file_path}")
                return {
                    "content": "",
                    "metadata": {"file_type": "xls", "status": "encrypted"},
                    "error": "涉密文件，跳过处理"
                }
            logger.warning(f"XLRD解析失败: {file_path} - {str(e)}")
        except Exception as e:
            logger.warning(f"xlrd+pandas提取失败: {file_path} - {str(e)}")

        # Windows COM 回退
        if self.is_windows:
            logger.debug(f"尝试使用COM接口提取: {file_path}")
            try:
                pythoncom.CoInitialize()
                excel_app = win32com.client.Dispatch("Excel.Application")
                excel_app.Visible = False
                excel_app.DisplayAlerts = False
                logger.debug(f"打开Excel文件: {file_path}")
                ...
                for worksheet in workbook.Sheets:
                    ...
                    if used_range and used_range.Rows.Count > 0:
                        ...
                        if len(df) > 5000:
                            sheet_content += "\n[注意: 表格过大，仅显示前5000行]"
                        content.append(sheet_content)
                logger.info(f"COM接口成功提取内容: {file_path}")
                ...
                if "encrypted" in str(e).lower() or "password" in str(e).lower():
                    logger.info(f"COM接口确认加密: {file_path}")
                    return {
                        "content": "",
                        "metadata": {"file_type": "xls", "status": "encrypted"},
                        "error": "涉密文件，跳过处理"
                    }
                logger.warning(f"COM接口提取失败: {file_path} - {str(e)}")
            finally:
                logger.debug(f"清理Excel进程: {file_path}")
                self._force_close_office_processes("EXCEL.EXE")
                pythoncom.CoUninitialize()
        else:
            logger.debug(f"非Windows环境，跳过COM回退: {file_path}")

        # 所有方法失败
        logger.info(f"提取XLS内容失败: {file_path}")
        return self._create_error_result("xls", "所有提取方法失败")
    ...
    def _extract_doc_content(self, file_path: str) -> Dict[str, Any]:
        """提取DOC文件内容，优化超时处理和资源管理"""
        logger.info(f"处理DOC文件: {file_path}")
        
        # 检查环境和资源
        if not self.is_windows:
            return self._create_error_result("doc", "仅支持Windows环境")
        if not self._init_word_app() or not self.word_app:
            return self._create_error_result("doc", "Word应用程序不可用")

        # 设置超时参数
        timeout_seconds = 10  # 可根据需要调整
        result = [None]  # 用于线程间传递结果
        ...
        def process_doc():
            ...
            try:
                ...
                doc = self.word_app.Documents.Open(
                    abs_path,
                    ReadOnly=True,
                    PasswordDocument="",  # 不提供密码，加密文件会抛异常
                    Visible=False,
                ...
                # 检查是否加密
                if doc.ProtectionType != -1:  # -1 表示无保护
                    result[0] = {
                        "content": "",
                        "metadata": {"file_type": "doc", "status": "encrypted"},
                        "error": "涉密文件，跳过处理"
                    }
                else:
        ...
        if process_thread.is_alive():
            ...
            self._force_close_office_processes("WINWORD.EXE")  # 强制清理残留进程
        ...
        if timeout_occurred[0]:
            return {
                "content": "",
                "metadata": {"file_type": "doc", "extractor": "win32com"},
                "error": f"处理超时 ({timeout_seconds}秒)"
            }
        elif exception[0]:
            ...
            logger.warning(f"处理DOC失败: {file_path} - {str(exception[0])}")
            return self._create_error_result("doc", f"Win32COM处理失败: {str(exception[0])}")
        elif result[0]:
            return result[0]
        
        # 意外情况
        return self._create_error_result("doc", "处理过程遇到未知错误")
    ...
    def extract_content(self, file_path: str, mime_type: str) -> Dict[str, Any]:
        """提取文件内容，优化处理流程，确保快速返回并避免卡顿"""
        logger.debug(f"开始提取文件内容: {file_path} (MIME: {mime_type})")

        # 基本检查
        if not Path(file_path).exists():
            logger.error(f"文件不存在: {file_path}")
            return {"content": "", "metadata": {}, "error": f"文件不存在: {file_path}"}
        ...
        try:
            ...
            skip_mime_types = [
                ...
                "font/ttf", "font/otf", "font/woff", "font/woff2", "application/x-executable"
            ]
            if mime_type in skip_mime_types or mime_type == "image/vnd.dwg":
                logger.info(f"跳过不支持的类型: {mime_type}")
                return self._create_error_result(mime_type, f"不支持的MIME类型或DWG文件: {mime_type}")
            ...
            if mime_type in self.extractors:
                ...
                try:
                    ...
                    logger.debug(f"提取器返回结果: {result}")
                    if result is None:
                        logger.error(f"提取器返回None: {file_path}")
                        return self._create_error_result(mime_type, "提取器返回无效结果")
                    # 无论成功还是失败（包括加密文件），直接返回
                    return result
                except Exception as e:
                    logger.warning(f"主要提取器 '{mime_type}' 发生异常: {str(e)}")
                    return self._create_error_result(mime_type, f"提取器异常: {str(e)}")

            # 如果没有专用提取器，尝试简单回退
            logger.info(f"未找到MIME类型 '{mime_type}' 的专用提取器，尝试回退")

            # MarkItDown 回退（简化并加强超时）
            if self.md is not None:
                logger.debug(f"尝试MarkItDown处理: {file_path}")
                timeout_seconds = 5
                result = [None]
                timeout_occurred = [False]
                ...
                def process_markdown():
                    try:
                        md_result = self.md.convert(file_path)
                        result[0] = {
                        ...
                        result[0] = self._create_error_result(mime_type, f"MarkItDown失败: {str(e)}")

                thread = threading.Thread(target=process_markdown)
                thread.daemon = True
                thread.start()
                thread.join(timeout_seconds)
                if thread.is_alive():
                    timeout_occurred[0] = True
                    logger.warning(f"MarkItDown处理超时 ({timeout_seconds}秒): {file_path}")
                if not timeout_occurred[0] and result[0]:
                    logger.debug(f"MarkItDown返回结果: {result[0]}")
                    return result[0]

            # 二进制提取作为最后手段
            logger.debug(f"尝试二进制提取: {file_path}")
            try:
                result = self._extract_binary_text_content(file_path, mime_type)
                logger.debug(f"二进制提取结果: {result}")
                if not result.get("error") and result.get("content"):
                    return result
            except Exception as e:
                logger.warning(f"二进制提取失败: {str(e)}")

            # 所有方法失败
            logger.info(f"所有提取方法失败: {file_path}")
            return self._create_error_result(mime_type, f"无法处理文件类型: {mime_type}")

        except Exception as e:
            logger.error(f"提取内容时发生严重错误: {str(e)} - 文件: {file_path}", exc_info=True)
            return self._create_error_result(mime_type, f"提取过程发生严重错误: {str(e)}")
...
class FileProcessor:
    ...
    def process_file(self, file_path: str) -> ProcessingResult:
        ...
        Returns:
            ProcessingResult: 文件处理结果对象
        """
        # 记录处理开始时间
        start_time = time.time()
        
        try:
            # 缓存中获取MIME类型，如果没有则重新检测
            mime_type = None
            ...
            try:
                ...
                if any(keyword in error_msg for keyword in encryption_keywords):
                    logger.info(f"检测到加密文件，跳过: {file_path}")
                    return ProcessingResult(
                        file_path=file_path,
                        mime_type=mime_type,
                        content={
                            "content": "",
                            "metadata": {"file_type": "encrypted", "skipped": True},
                        },
                        sensitive_words=[],
                        error="加密文件，已跳过",
                        processing_time=time.time() - start_time,
