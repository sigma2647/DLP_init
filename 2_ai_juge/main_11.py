...
class ResultExporter:
    ...
    def __init__(self, include_content=True, max_content_length=10000):
        """
        初始化结果导出器
        
        Args:
            include_content: 是否在Excel中包含文本内容
            max_content_length: 导出内容的最大长度限制
        """
    ...
    def export_to_json(self, results: List[ProcessingResult], output_path: str):
        ...
        for r in results:
            # 获取内容预览
            content_preview = self._get_content_preview(r, 500)
            ...
            result_dict = {
                ...
                "sensitive_words": [
                    {"word": w, "positions": p, "count": len(p)} for w, p in r.sensitive_words
                ],
                "has_sensitive_content": bool(r.sensitive_words),
                "error": r.error,
            ...
            # 添加元数据（如果可用）
            if isinstance(r.content, dict) and "metadata" in r.content:
                result_dict["metadata"] = r.content["metadata"]
        ...
        self.export_count += 1
        logger.info(f"已导出 {len(results)} 条结果到JSON文件: {output_path}")
    ...
    def export_to_excel(self, results: List[ProcessingResult], output_path: str):
        ...
        try:
            ...
            for idx, r in enumerate(results, 1):
                # 生成敏感词统计文本
                if r.sensitive_words:
                    sensitive_words_text = "; ".join(
                        [f"{w}({len(p)}次)" for w, p in r.sensitive_words]
                    )
                else:
                    sensitive_words_text = "无"
                    
                # 创建每行数据    
                row_data = {
                    "序号": idx,
                    "文件路径": r.file_path,
                    "文件名": os.path.basename(r.file_path),
                    "文件类型": r.mime_type,
                    "是否敏感": "是" if r.sensitive_words else "否",
                    "敏感词统计": sensitive_words_text,
                ...
                if isinstance(r.content, dict) and "metadata" in r.content:
                    ...
                    if "file_type" in metadata:
                        row_data["文件实际类型"] = metadata["file_type"]
                    if "extractor" in metadata:
                        row_data["提取器"] = metadata["extractor"]
                    if "paragraphs" in metadata:
                        row_data["段落数"] = metadata["paragraphs"]
                    if "pages" in metadata:
                        row_data["页数"] = metadata["pages"]
                    if "sheets" in metadata:
                        row_data["工作表数"] = metadata["sheets"]
                    if "slides" in metadata:
                        row_data["幻灯片数"] = metadata["slides"]
                ...
                content_data = [
                    {
                        ...
                        "文件名": os.path.basename(r.file_path),
                        "文件类型": r.mime_type,
                        "内容": self._get_truncated_content(r),
                    }
                    for idx, r in enumerate(results, 1)
                ]
            ...
            for idx, r in enumerate(results, 1):
                if r.sensitive_words:
                    for word, positions in r.sensitive_words:
            ...
            summary = self._generate_summary(results)
            summary_stats = [
                {"统计项": k, "值": v} for k, v in summary.items()
            ]
            ...
            with pd.ExcelWriter(output_path, engine="openpyxl") as writer:
                ...
                # 敏感内容工作表（如果有）
                if sensitive_data:
                    pd.DataFrame(sensitive_data).to_excel(
                        writer, sheet_name="敏感内容", index=False
                    )
    ...
    def export_to_csv(self, results: List[ProcessingResult], output_path: str):
        """
        导出结果到CSV文件
        ...
        try:
            ...
            for idx, r in enumerate(results, 1):
                row_data = {
                    ...
                    "文件类型": r.mime_type,
                    "是否敏感": "是" if r.sensitive_words else "否",
                    "敏感词": "; ".join([w for w, _ in r.sensitive_words]) if r.sensitive_words else "",
                    "处理时间(秒)": round(r.processing_time, 3),
                    "错误信息": r.error or "",
                }
    ...
    def _generate_summary(self, results: List[ProcessingResult]) -> Dict:
        """
        生成处理结果的摘要统计
        ...
        sensitive_files = sum(1 for r in results if r.sensitive_words)
        ...
        for r in results:
            for word, positions in r.sensitive_words:
                sensitive_words[word] = sensitive_words.get(word, 0) + len(positions)
                
        # 按出现次数排序敏感词
        sorted_sensitive_words = dict(sorted(
            sensitive_words.items(), 
        ...
        summary = {
            "总文件数": total_files,
            "敏感文件数": sensitive_files,
            "敏感文件比例": f"{(sensitive_files / total_files * 100):.2f}%" if total_files > 0 else "0%",
            "出错文件数": errors,
            "总处理时间(秒)": f"{total_time:.2f}",
            "平均处理时间(秒/文件)": f"{avg_time:.2f}",
            "处理日期": datetime.now().strftime("%Y-%m-%d %H:%M"),
        }
        
        # 添加文件类型统计
        for mime_type, count in sorted(mime_types.items(), key=lambda x: x[1], reverse=True)[:10]:
            summary[f"文件类型: {mime_type}"] = count
            
        # 添加敏感词统计（最多10个）
        for word, count in list(sorted_sensitive_words.items())[:10]:
            summary[f"敏感词: {word}"] = count
    ...
    def _get_truncated_content(self, result: ProcessingResult) -> str:
        """
        从处理结果中获取截断的完整内容
...
class ResultMonitor:
    ...
    def __init__(self, output_csv: str = "processing_results.csv", verbose: bool = True):
        """
        初始化监控器
        
        Args:
            output_csv: CSV输出文件路径
            verbose: 是否启用详细输出
        """
    ...
    def _init_csv(self):
        ...
        with open(self.output_csv, "w", encoding="utf-8-sig", newline="") as f:
            writer = csv.writer(f)
            writer.writerow(["序号", "文件名", "文件路径", "识别类型", "敏感词", "处理时间(秒)", "错误信息", "处理时间戳"])
    ...
    def record_result(self, result: ProcessingResult):
        ...
        if result.sensitive_words:
            self.sensitive_files += 1
            
            # 统计敏感词
            for word, positions in result.sensitive_words:
                if word not in self.sensitive_words:
                    self.sensitive_words[word] = 0
                self.sensitive_words[word] += len(positions)
        ...
        sensitive_words_text = "; ".join([f"{w}({len(p)}次)" for w, p in result.sensitive_words]) if result.sensitive_words else ""
    ...
    def _print_progress(self, result: ProcessingResult):
        ...
        remaining_time = remaining_files * avg_time_per_file if avg_time_per_file > 0 else 0
        ...
        # 打印进度条
        print(f"\r处理进度: [{bar}] {progress_pct:.1f}% ({self.processed_files}/{self.total_files}) ", end="")
        
        # 每隔一定数量的文件，打印详细信息
        if self.processed_files % 10 == 0 or result.sensitive_words or result.error:
            print()  # 换行
            print(f"[{datetime.now().strftime('%H:%M:%S')}] 文件: {os.path.basename(result.file_path)}")
            ...
            if result.sensitive_words:
                print(f"  敏感词: {'; '.join([f'{w}({len(p)}次)' for w, p in result.sensitive_words])}")
    ...
    def print_summary(self):
        ...
        print("\n" + "=" * 80)
        print(f"处理完成！总共处理 {self.processed_files} 个文件，总耗时 {time_str}")
        print(f"敏感文件: {self.sensitive_files} ({(self.sensitive_files / self.processed_files * 100):.1f}%)")
        print(f"出错文件: {self.error_files} ({(self.error_files / self.processed_files * 100):.1f}%)")
        
        # 打印文件类型分布
        print("\n文件类型分布:")
        for mime_type, count in sorted(self.file_types.items(), key=lambda x: x[1], reverse=True)[:10]:
            print(f"  {mime_type}: {count} 个文件")
            
        # 打印敏感词统计
        if self.sensitive_words:
            print("\n敏感词统计:")
            for word, count in sorted(self.sensitive_words.items(), key=lambda x: x[1], reverse=True)[:10]:
                print(f"  {word}: {count} 次")
                
        print(f"\n详细结果已保存至: {self.output_csv}")
        print("=" * 80)
...
class FileProcessor:
    ...
    def __init__(
        ...
        Args:
            ...
            model_path: BERT模型文件路径
            use_model: 是否使用BERT模型
            monitor_output: 监控输出CSV文件路径
            chunk_size: 每批处理的文件数量
            max_workers: 最大工作线程数
            is_windows: 是否为Windows系统
            include_content: 是否在导出结果中包含文本内容
            max_content_length: 导出内容的最大长度限制
            verbose: 是否启用详细输出
        """
        ...
        self.exporter = ResultExporter(include_content=include_content, max_content_length=max_content_length)
        self.monitor = ResultMonitor(monitor_output, verbose=verbose)
        self.chunk_size = chunk_size
        self.max_workers = max_workers or min((os.cpu_count() or 1) * 2, 16)  # 限制最大线程数
        self.is_windows = is_windows
        self.verbose = verbose
        ...
        # 初始化日志
        logger.info(f"FileProcessor 初始化完成，最大工作线程数: {self.max_workers}")
        if self.verbose:
            print(f"初始化完成 - 最大线程数: {self.max_workers}")
            if self.checker.mode == 'bert':
                print(f"敏感内容检测模式: BERT模型 ({self.checker.device})")
            elif self.checker.mode == 'yaml':
                print(f"敏感内容检测模式: YAML配置 ({len(self.checker.all_keywords)} 个关键词)")
            else:
                print("敏感内容检测模式: 空模式")
            ...
            for root, dirs, files in os.walk(directory):
                ...
                for file_name in files:
                    try:
                        ...
                        # 跳过Office内部流文件
                        if self.detector._is_internal_stream(file_name):
                            continue
                        ...
                        # 跳过已知的二进制格式
                        ext = Path(file_path).suffix.lower()
                        if ext in self.detector.SKIP_EXTENSIONS:
                            continue
                        ...
                        logger.warning(f"访问文件/目录出错 {file_name}: {e}")
                        continue
        except (PermissionError, OSError) as e:
    ...
    def _process_file_batch(self, file_paths: List[str]) -> List[ProcessingResult]:
        ...
        for fp in office_files:
            try:
                ...
                logger.error(f"处理Office文件失败 {fp}: {str(e)}")
                # 创建错误结果
                error_result = ProcessingResult(
                    file_path=fp,
                    mime_type=self._mime_cache.get(fp, "unknown"),
    ...
    def process_directory(self, directory: str) -> List[ProcessingResult]:
        ...
        # 获取文件列表
        all_files = self.detector.get_all_files(directory)
        total_files = len(all_files)
        self.total_files = total_files
        self.monitor.set_total_files(total_files)
        ...
        try:
            ...
            for chunk in file_chunks:
                chunk_results = self._process_file_batch(chunk)
                results.extend(chunk_results)
        except KeyboardInterrupt:
            print("\n用户中断处理...")
            self._handle_interruption(results)
        except Exception as e:
            ...
            # 导出结果
            output_prefix = f"敏感内容检测_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
            if results:
                try:
                    self.exporter.export_to_json(results, f"{output_prefix}.json")
                    self.exporter.export_to_excel(results, f"{output_prefix}.xlsx")
                    self.exporter.export_to_csv(results, f"{output_prefix}.csv")
                except Exception as e:
                    logger.error(f"导出结果失败: {e}")

        return results

    def _handle_interruption(self, results: List[ProcessingResult]):
        """处理中断情况，保存已有结果"""
        if results:
            try:
                interrupt_prefix = f"中断保存_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
                self.exporter.export_to_json(results, f"{interrupt_prefix}.json")
                self.exporter.export_to_excel(results, f"{interrupt_prefix}.xlsx")
                print(f"\n已保存中断时的处理结果至 {interrupt_prefix}.json 和 {interrupt_prefix}.xlsx")
            except Exception as e:
        ...
        try:
            ...
            if not os.path.exists(file_path):
                return ProcessingResult(
                    file_path=file_path,
                    mime_type="unknown",
                    content={"content": "", "error": f"文件不存在: {file_path}"},
                    sensitive_words=[],
                    error=f"文件不存在: {file_path}",
            ...
            # 定义常规文件的条件
            if (mime_type in skip_mime_types or 
            ...
            try:
                ...
                return ProcessingResult(
                    file_path=file_path,
                    mime_type=mime_type,
                    content={"content": "", "metadata": {"file_type": mime_type}, "error": f"提取内容时发生严重错误: {str(extract_error)}"},
                    sensitive_words=[],
            ...
            # 敏感内容检查
            try:
                sensitive_words = self.checker.check_file_content(content)
            ...
            return ProcessingResult(
                ...
                content={"content": "", "metadata": {"file_type": "unknown"}, "error": error_msg},
                sensitive_words=[],
                error=error_msg,
                processing_time=time.time() - start_time,
            )
...
def main():
    ...
    parser.add_argument(
        "--output", default="敏感内容检测结果", help="输出结果文件名前缀(不含扩展名)"
    )
    parser.add_argument(
        "--chunk-size", type=int, default=100, help="每批处理的文件数量"
    )
    parser.add_argument(
        "--workers", type=int, default=None, help="最大工作线程数"
    )
    parser.add_argument(
        "--no-windows",
        action="store_true",
    ...
    try:
        # 配置日志
        log_file = f"敏感内容检测_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
        logging.basicConfig(
            level=logging.INFO,
            format="%(asctime)s - %(levelname)s - %(message)s",
            handlers=[
        ...
        processor = FileProcessor(
            ...
            monitor_output=f"{args.output}_处理记录.csv",
        ...
        if os.path.isfile(args.path):
            ...
            results = [result]
            processor.monitor.record_result(result)
        else:
            # 处理目录
        ...
        if results:
            ...
            processor.exporter.export_to_csv(results, f"{args.output}.csv")
        
        # 计算总耗时
        total_time = time.time() - start_time
        mins, secs = divmod(int(total_time), 60)
        hours, mins = divmod(mins, 60)
        time_str = f"{hours}小时{mins}分钟{secs}秒" if hours > 0 else f"{mins}分钟{secs}秒"
        
        if not args.quiet:
            print(f"\n处理完成，总共处理 {len(results)} 个文件，总耗时 {time_str}")
            print(f"详细结果已保存至: {args.output}.xlsx, {args.output}.json, {args.output}.csv")
            print(f"日志文件: {log_file}")
            print("=" * 80)
        
        logger.info(f"处理完成，总共处理 {len(results)} 个文件，总耗时 {time_str}")
