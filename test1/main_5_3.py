...
class FileProcessor:
    ...
    def process_file(self, file_path: str) -> ProcessingResult:
        ...
        try:
            ...
            # 对于 .dwg 文件，直接返回常规文件结果
            if ext == '.dwg':
                logger.info(f"跳过 .dwg 文件提取: {file_path}")
                return ProcessingResult(
                    file_path=file_path,
                    mime_type=mime_type,
                    content={'content': '', 'metadata': {'file_type': 'dwg'}, 'skipped': True},
                    sensitive_words=[],
                    error=None,
            ...
            sensitive_words = sensitive_result  # 直接使用返回的 List[Tuple[str, List[int]]]
